...
- `/clanadd <user> <clan> [send_welcome]` - Add a user to a clan and assign all necessary roles
  - `user`: The Discord user to add to the clan
  - `clan`: The clan to add the user to (grp9/moon)
  - `send_welcome`: Whether to send a welcome message (optional, default: true)
  - Requires Admin/Officer role
  - Automatically assigns clan role and additional configured roles
  - Sends a customizable welcome message via DM

### Configuration

#### Role Configuration
Additional roles that should be assigned when adding a user to a clan can be configured in the `.env` file:

```env
# Additional roles for Clan 1 (comma-separated role IDs)
CLAN1_ADDITIONAL_ROLES=role_id1,role_id2

# Additional roles for Clan 2 (comma-separated role IDs)
CLAN2_ADDITIONAL_ROLES=role_id3,role_id4
```

#### Welcome Messages
Welcome messages for each clan can be customized in `src/messages/welcome_messages.py`. The messages support:
- Clan-specific information
- Raid schedules
- Important channel information
- Custom formatting and emojis
...
- `Manage Roles` - Required to assign roles to users
- `Send Messages` - To send command responses and error messages
- `Send Messages in Threads` - For thread support
- `View Channels` - To see and respond to commands
- `Read Message History` - For command context
- `Use Slash Commands` - To register and use commands

### Important Notes
- The bot's role must be HIGHER in the role hierarchy than the roles it needs to manage
  - Example hierarchy (top to bottom):
    1. Admin roles
    2. Bot role(s) ← Must be here or higher
    3. Managed roles (clan roles, etc.)
    4. Member roles
- Administrator permissions are NOT required for the bot to function
- For role management to work, ensure the bot has the necessary role hierarchy position
- Common permission error: "❌ I don't have permission to add roles to this user" usually means the bot's role is too low in the hierarchy 